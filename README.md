...
- bot.py
  Script principal. Inicializa o ChatBot, treina usando ListTrainer a partir do corpus limpo e abre um loop de conversa no terminal. Inclui pequenas melhorias de ergonomia (banner, horário, comandos utilitários e tratamento de interrupção). :contentReference[oaicite:0]{index=0}

- cleaner.py
  Pipeline de limpeza do corpus. Faz leitura robusta (evita erros de encoding no Windows), remove metadados do export (data/hora/autor) via expressão regular e filtra linhas irrelevantes (mídia omitida, vazios). :contentReference[oaicite:1]{index=1}

- chatpot.sqlite3 / db.sqlite3 / db.sqlite3-wal / db.sqlite3-shm
  Arquivos SQLite gerados pelo ChatterBot (storage). Em SQLite com WAL, os arquivos -wal e -shm podem aparecer durante execução. Esses artefatos são recriáveis e, em geral, devem ficar no .gitignore em repositórios públicos. :contentReference[oaicite:2]{index=2}
...
A função remove_chat_metadata remove o prefixo de metadados e preserva apenas o conteúdo da mensagem. Em seguida, remove_non_message_text filtra linhas vazias e marcadores típicos de export (ex.: <Media omitted>). :contentReference[oaicite:3]{index=3}
...
Coloque o arquivo de export (ex.: chat.txt) na raiz do projeto e garanta que a variável CORPUS_FILE em bot.py aponte para ele. :contentReference[oaicite:4]{index=4}
...
Esses comandos são tratados no loop principal, antes de chamar chatbot.get_response(...). :contentReference[oaicite:5]{index=5}

O que este projeto demonstra (para avaliação técnica)

- Higienização de dados textuais “do mundo real”
  Exportações de chat carregam ruído (metadados, linhas vazias, mídia omitida). O cleaner.py centraliza esse pré-processamento e mantém o treinamento desacoplado do formato bruto. :contentReference[oaicite:6]{index=6}

- Robustez de encoding em Windows
  A leitura do corpus tenta encodings comuns (utf-8-sig, utf-8, cp1252, latin-1) e ainda possui fallback para “replace” em caso extremo, evitando travamentos por UnicodeDecodeError. :contentReference[oaicite:7]{index=7}
...
Isso mantém o repositório focado em código e evita versionar dados sensíveis e bancos recriáveis. :contentReference[oaicite:9]{index=9}
